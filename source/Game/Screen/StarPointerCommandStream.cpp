...
namespace NrvStarPointerCommandStream {
    FULL_NERVE(HostTypeNrvWait, StarPointerCommandStream, Wait);
    FULL_NERVE(HostTypeNrvSignal, StarPointerCommandStream, Signal);
}  // namespace NrvStarPointerCommandStream

namespace {
    static Color8 hOutColor = Color8(0xFF, 0x00, 0x00, 0xFF);
}  // namespace

// TODO: probably an MR function
inline f32 toRadian(f32 angle) {
    f32 x = (180.0f / JMath::PI);
    return angle * x;
}

namespace MR {
    // This got removed from SMG2
    void setPaneScale(const LayoutActor* pActor, register f32 x, register f32 y, const char* pPaneName) {
        register nw4r::lyt::Pane* pPane = pActor->getLayoutManager()->getPane(pPaneName);
        __asm {
            stfs x, 0x44(pPane)
            stfs y, 0x48(pPane)
        }
...
void StarPointerCommandStream::calcPose(const TVec2f& rPos, const TVec2f& rInsidePos) {
    ...
    MR::setPaneRotate(this, 0.0f, 0.0f, toRadian(JMath::sAtanTable.atan2_(diff.x, diff.y)), "Arrow");
}
